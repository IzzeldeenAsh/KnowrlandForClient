...
class NotificationService {
  ...
  private async fetchNotificationsFromAPI(locale: string): Promise<Notification[]> {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/account/notification`, {
        headers: {
  ...
  public async markNotificationAsRead(id: string, locale: string = 'en'): Promise<boolean> {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/account/notification/read/${id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  public async markAllNotificationsAsRead(locale: string = 'en'): Promise<boolean> {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/account/notification/read`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
