...
- `apiBaseUrl`: In development, uses `https://api.knoldg.co`; in production, uses `https://api.knoldg.co`
...
```typescript
const response = await fetch('https://api.knoldg.co/api/platform/industries', {...});
```
...
1. When using the helper functions, remove the base URL and start the path with a slash:
   - ❌ `getApiUrl('https://api.knoldg.co/api/platform/industries')` 
   - ✅ `getApiUrl('/api/platform/industries')`

2. The environment is automatically detected based on Node.js' `process.env.NODE_ENV`.

3. When running in development, APIs will use `https://api.knoldg.co` and app redirects will use `https://app.knoldg.com`.
