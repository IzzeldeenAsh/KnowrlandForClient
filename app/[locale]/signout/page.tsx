...
export default function SignoutPage() {
  ...
  useEffect(() => {
    ...
    // Compare against the configured dashboard host — hardcoding the production
    // host made this always true on staging and in dev.
    const angularHost = (() => {
      try {
    ...
    const shouldNotifyAngularApp = !(angularHost && redirectUri?.includes(angularHost));
    ...
    if (shouldNotifyAngularApp) {
      ...
      angularLogoutFrame.src = `${dashboardUrl}/auth/logout?redirect_uri=${encodeURIComponent(window.location.href)}`;
