...
export async function fetchAutocomplete(
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/platform/search/autocomplete?keyword=${encodeURIComponent(keyword)}`, {
      headers
    });
...
export async function fetchStatisticsPerType(
  ...
  try {
    const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/platform/search/statistics-per-type`);
...
export async function fetchSearchResults(
  ...
  try {
    ...
    const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/platform/search`);
