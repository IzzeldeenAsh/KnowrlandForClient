...
import { useTopicDetails } from '@/hooks/industries/useTopicDetails';
...
export default function TopicPage() {
  ...
  const { topic, isLoading, error } = useTopicDetails({ id, slug });
  ...
  useEffect(() => {
    ...
    if (topic.slug && topic.slug !== slug) {
      router.replace(`/${locale}/topic/${id}/${topic.slug}`);
      return;
    }
    fetchBreadcrumb('topic', parseInt(id), locale)
      .then((items) =>
        setBreadcrumbItems(items.map((item) => ({ label: item.label, href: item.url })))
      )
      .catch(() => setBreadcrumbItems([]));
  }, [topic, id, slug, locale, router]);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center px-4">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            {locale === 'ar' ? 'تعذر تحميل بيانات الموضوع' : 'Unable to load topic details'}
          </h2>
      ...
      <main className="min-h-screen bg-gray-50" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <section className="max-w-container relative mx-auto mt-10 w-full px-4 sm:px-6 lg:px-8 pb-12">
          <div className="max-w-6xl mx-auto">
            ...
            >
              {messages?.topic?.knowledge || (locale === 'ar' ? 'المستندات' : 'Insights')}
            </h2>
