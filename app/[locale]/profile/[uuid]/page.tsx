...
export default function ProfilePage() {
      ...
      try {
        ...
        if (entityType === "insighter") {
          ...
          let response = await fetch(
            `https://api.knoldg.co/api/platform/insighter/profile/${uuid}`,
            {
              headers: {
          ...
          if (response.ok) {
            ...
            response = await fetch(
              `https://api.knoldg.co/api/platform/company/profile/${uuid}`,
              {
                headers: {
                  "Content-Type": "application/json",
                  Accept: "application/json",
      ...
      try {
        ...
        let url =
          entityType === "insighter"
            ? `https://api.knoldg.co/api/platform/insighter/knowledge/${uuid}?page=${knowledgePage}&per_page=12`
            : `https://api.knoldg.co/api/platform/company/knowledge/${uuid}?page=${knowledgePage}&per_page=12`;
  ...
  const fetchMeetingAvailability = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.knoldg.co/api/account/meeting/available/hours/${uuid}`,
        {
  ...
  const submitBookMeeting = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.knoldg.co/api/account/meeting/book/${uuid}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
