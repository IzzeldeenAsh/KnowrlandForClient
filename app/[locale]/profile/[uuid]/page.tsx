...
function ProfilePageContent() {
  ...
  useEffect(() => {
    const fetchAllKnowledgeTotal = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/platform/company/knowledge/${uuid}?page=1&per_page=1`,
          {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
  ...
  useEffect(() => {
    const fetchInsighterStatistics = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/platform/insighter/profile/statistics/${uuid}`,
          {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
  ...
  const fetchMeetingAvailability = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/platform/insighter/meeting/available/hours/${uuid}`,
        {
