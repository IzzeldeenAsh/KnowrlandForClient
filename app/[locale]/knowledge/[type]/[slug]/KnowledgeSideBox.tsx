...
const KnowledgeSideBox = ({
  ...
  const handleReadLaterToggle = async () => {
    ...
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_API_URL}/api/account/favorite/knowledge/${knowledgeSlug}`
            ...
            {!isOwner && (
              <>
                {purchased_status === 'purchased' ? (
                  <button
                    onClick={() => window.location.href = (process.env.NEXT_PUBLIC_DASHBOARD_URL || `${process.env.NEXT_PUBLIC_DASHBOARD_URL}`) + '/app/insighter-dashboard/my-downloads'}
