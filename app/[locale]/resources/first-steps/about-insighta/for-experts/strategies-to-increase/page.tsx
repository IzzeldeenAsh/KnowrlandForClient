...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: isArabic ? 'استراتيجيات لزيادة | إنسايتا' : 'Strategies to Increase | Insighta',
    description: isArabic 
      ? 'تعرف على استراتيجيات الزيادة'
      : 'Learn about strategies to increase',
  }
}

export default async function StrategiesToIncreasePage({ params }: { params: Promise<{ locale: string }> }) {
  const resolvedParams = await params
  const locale = resolvedParams.locale
          ...
          <div className="text-center mb-12 md:mb-16">
            <div className={`flex flex-col align-center justify-center gap-2 ${isRTL ? 'text-right' : 'text-left'} text-left`} style={{lineHeight: '1.3'}}>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-gray-900">
                {locale === 'ar' ? 'استراتيجيات لزيادة' : 'Strategies to Increase'}
              </h1>
