...
export default function TagsTab() {
  ...
  const fetchIndustries = useCallback(async (signal?: AbortSignal) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/common/setting/industry/list`, {
        method: 'GET',
        cache: 'no-store',
        signal,
        headers: {
  ...
  const fetchTags = useCallback(
    async (page = 1, signal?: AbortSignal) => {
      ...
      try {
        ...
        const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/tag`);
  ...
  const submitUpsert = async (payload: TagUpsertPayload) => {
    ...
    try {
      ...
      const url =
        modalMode === 'create'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/tag`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/tag/${selectedTag?.id ?? ''}`;
  ...
  const deleteTag = async (tag: TagRecord) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/tag/${tag.id}`, {
        method: 'DELETE',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
      });
