...
export default function TopicsTab() {
  ...
  const fetchIndustries = useCallback(async (signal?: AbortSignal) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/common/setting/industry/tree-list`, {
        method: 'GET',
        cache: 'no-store',
        signal,
        headers: {
  ...
  const fetchTopics = useCallback(
    async (page = 1, signal?: AbortSignal) => {
      ...
      try {
        ...
        const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/topic`);
  ...
  const submitUpsert = async (payload: TopicUpsertPayload) => {
    ...
    try {
      ...
      const url =
        modalMode === 'create'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/topic`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/topic/${selectedTopic?.id ?? ''}`;
  ...
  const deleteTopic = async (topic: TopicRecord) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/topic/${topic.id}`, {
        method: 'DELETE',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
      });
