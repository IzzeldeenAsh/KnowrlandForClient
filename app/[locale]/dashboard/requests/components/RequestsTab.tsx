...
export default function RequestsTab() {
  ...
  const fetchRequests = useCallback(
    async (page = 1, perPageValue = perPage, signal?: AbortSignal) => {
      ...
      try {
        ...
        const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/request`);
  ...
  const fetchVerificationQuestions = useCallback(async (signal?: AbortSignal) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/common/setting/verification-question/list`, {
        method: 'GET',
        cache: 'no-store',
        signal,
        headers: {
    ...
    if (typeKey === 'verified_company') {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/verification/question/${request.id}`,
          {
            method: 'GET',
            cache: 'no-store',
            headers: buildAuthHeaders(token),
  ...
  const submitVerificationAnswers = async (requestId: number, answers: VerificationAnswerPayload[]) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/action/company/verified/question/${requestId}`,
        {
          method: 'POST',
          cache: 'no-store',
          headers: buildAuthHeaders(token),
  ...
  const approveRequest = async (requestId: number, staffNotes: string, status: 'approved' | 'declined') => {
    ...
    try {
      ...
      let url = '';
      if (typeKey === 'activate_company') {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/action/company/activate/${requestId}`;
      } else if (typeKey === 'deactivate_company') {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/action/company/deactivate/${requestId}`;
      } else if (typeKey === 'verified_company') {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/action/company/verified/${requestId}`;
      } else if (typeKey === 'deactivate_delete_company' || typeKey === 'deactivate_delete_insighter') {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/admin/request/action/company/deactivate-delete/${requestId}`;
