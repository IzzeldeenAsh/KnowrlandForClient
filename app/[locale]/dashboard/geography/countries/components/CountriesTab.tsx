...
export default function CountriesTab() {
  ...
  const fetchCountries = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/common/setting/country/list`, {
        method: 'GET',
        cache: 'no-store',
        signal,
        headers: {
  ...
  const fetchRegions = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/region`, {
        method: 'GET',
        cache: 'no-store',
        signal,
  ...
  const submitUpsert = async (payload: CountryUpsertPayload) => {
    ...
    try {
      ...
      const url =
        modalMode === 'create'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/country`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/country/${selectedCountry?.id ?? ''}`;
  ...
  const deleteCountry = async (country: CountryRecord) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/country/${country.id}`, {
        method: 'DELETE',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
      });
