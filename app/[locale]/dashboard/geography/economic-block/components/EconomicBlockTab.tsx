...
export default function EconomicBlockTab() {
  ...
  const fetchBlocs = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/economic-bloc`, {
        method: 'GET',
        cache: 'no-store',
        signal,
  ...
  const submitUpsert = async (payload: LocalizedNamePayload) => {
    ...
    try {
      ...
      const url =
        modalMode === 'create'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/economic-bloc`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/economic-bloc/${selectedBloc?.id ?? ''}`;
  ...
  const deleteBloc = async (bloc: EconomicBlocRecord) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/economic-bloc/${bloc.id}`, {
        method: 'DELETE',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
      });
