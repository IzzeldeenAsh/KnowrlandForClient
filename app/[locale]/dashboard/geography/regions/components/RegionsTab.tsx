...
export default function RegionsTab() {
  ...
  const submitUpsert = async (payload: LocalizedNamePayload) => {
    ...
    try {
      ...
      const url =
        modalMode === 'create'
          ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/region`
          : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/region/${selectedRegion?.id ?? ''}`;
  ...
  const deleteRegion = async (region: RegionRecord) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/setting/region/${region.id}`, {
        method: 'DELETE',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
      });
