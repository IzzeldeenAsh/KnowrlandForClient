...
export default function AdminSalesTab() {
  ...
  const fetchTotalStatistics = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/sales/revenue/statistics`, {
        method: 'GET',
        cache: 'no-store',
        signal,
  ...
  const fetchPeriodStatistics = useCallback(
    async (period: PeriodType, signal?: AbortSignal) => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/sales/revenue/period/statistics?per_time=${period}`,
        {
          method: 'GET',
          cache: 'no-store',
          signal,
