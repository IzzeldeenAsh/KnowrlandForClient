...
<html>
  ...
  <body>
	    <div class="page">
	      <div class="header">
	        <div class="brand">
	          <img class="logo" alt="Insighta" src="${process.env.NEXT_PUBLIC_DASHBOARD_URL}/assets/media/logos/custom-2.svg" />
	          <div class="heading">
...
export default function TransferFormTab({ insighterId }: { insighterId: string }) {
  ...
  const fetchForm = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/fund/insighter/payment/manual/${insighterId}`, {
        method: 'GET',
        cache: 'no-store',
  ...
  const onSendEmail = async (email: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/fund/insighter/send/transfer-form-email/${insighterId}`, {
        method: 'POST',
        cache: 'no-store',
        headers: buildAuthHeaders(token),
  ...
  const onSaveReceipt = async (values: ReceiptFormValues) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/fund/insighter/wired-transfer/store/${insighterId}`, {
        method: 'POST',
        cache: 'no-store',
        headers,
        body: formData,
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <div className="flex items-start justify-between gap-6">
                <div className="flex min-w-0 items-start gap-4">
                  <img
                    src={`${process.env.NEXT_PUBLIC_DASHBOARD_URL}/assets/media/logos/custom-2.svg`}
                    alt="Insighta"
