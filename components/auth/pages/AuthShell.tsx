...
export function AuthShell({ locale, title, subtitle, children }: { locale: string; title: string; subtitle?: string; children: React.ReactNode }) {
  ...
  return <main className="auth-screen" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
...
export function PasswordInput({ label, locale, name = 'password', newPassword = false }: { label: string; locale: string; name?: string; newPassword?: boolean }) {
  const [visible, setVisible] = useState(false);
  return <label className="auth-field" htmlFor={name}>{label}<span className="auth-password">
    <input id={name} name={name} type={visible ? 'text' : 'password'} required minLength={8} autoComplete={newPassword ? 'new-password' : 'current-password'} dir="ltr" />
