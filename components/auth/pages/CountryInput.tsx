...
export default function CountryInput({ countries, locale, hideLabel = false }: { hideLabel?: boolean; countries: Country[]; locale: string }) {
  ...
  return <div className={`auth-field auth-country${hideLabel ? ' auth-country-compact' : ''}`} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) { setOpen(false); setQuery(''); } }}>
