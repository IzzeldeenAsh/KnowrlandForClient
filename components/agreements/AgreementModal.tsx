...
const AgreementModal: React.FC<AgreementModalProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchAgreement = async () => {
      ...
      setCanAccept(false);
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/common/setting/guideline/type/last/${computedAgreementType}`, {
          headers: {
  ...
  const onAccept = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/account/agreement/accept/${finalUuid}`, {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
          'Accept-Language': (locale as string) || 'en',
