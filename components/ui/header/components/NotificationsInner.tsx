...
export default function NotificationsInner({
      ...
      >
        <div className="flex items-start">
          <div className={`flex-shrink-0 ${isRTL ? 'ml-3' : 'mr-3'}`}>
            <div className={`h-12 w-12 rounded-md flex items-center justify-center ${colorClasses.bg} ${colorClasses.text}`}>
  ...
  const handleNotificationClick = useCallback(
    async (notification: Notification) => {
      ...
      // Fire-and-forget mark-as-read first, then redirect immediately
      void onNotificationClick(notification.id).catch(() => {})
      if (url) {
